...
namespace modern_coro
{
    ...
    void WorkStealingScheduler::worker_thread()
    {
        ...
        {
            LOG_MODULE_WARN(logger::modules::SCHEDULER, "Thread index out of range");
            unregister_thread();
            return;
        }
    ...
    void WorkStealingScheduler::print_work_stealing_stats() const
    {
        ...
        LOG_INFO("\n=== 工作窃取调度器统计信息 ===");
        LOG_INFO("基本统计:");
        LOG_INFO("  活跃协程数: {}", scheduler_stats.active_coroutines);
        LOG_INFO("  队列任务数: {}", scheduler_stats.queued_tasks);
        LOG_INFO("  总调度数: {}", scheduler_stats.total_scheduled);
        LOG_INFO("  总完成数: {}", scheduler_stats.total_completed);

        LOG_INFO("\n工作窃取统计:");
        LOG_INFO("  总窃取尝试: {}", stats.total_steals);
        LOG_INFO("  成功窃取: {}", stats.successful_steals);
        LOG_INFO("  失败窃取: {}", stats.failed_steals);
        LOG_INFO("  窃取成功率: {:.2f}%", stats.steal_success_rate * 100);

        LOG_INFO("\n每线程统计:");
        for (size_t i = 0; i < stats.per_thread_executed.size(); ++i)
        {
            LOG_INFO("  线程 {}: 执行={}, 被窃取={}", i, stats.per_thread_executed[i], stats.per_thread_stolen[i]);
        }
    }
