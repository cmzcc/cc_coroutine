...
#define MODERN_CORO_DEBUG 0
    ...
    void Scheduler::stop()
    {
        ...
        while (!is_idle())
        {
            ...
            if (elapsed.count() > 10)
            { // 最多等待10秒
                LOG_MODULE_WARN(modules::SCHEDULER,
                                "Scheduler stop timed out waiting for {} active coroutines",
                                get_active_coroutines());
                break;
            }
        }
    ...
    void Scheduler::schedule(std::function<void()> func)
    {
        ...
        total_scheduled_.fetch_add(1, std::memory_order_relaxed);
        {
            ...
            if (tasks_.size() >= max_queue_size_)
            {
                ...
                // 这里选择丢弃并记录
                LOG_MODULE_WARN(modules::SCHEDULER, "Task queue overflow (size={}), dropping task", tasks_.size());
                return;
            }
            ...
            try {
                ...
                LOG_MODULE_ERROR(modules::SCHEDULER, "Task execution failed: {}", e.what());
                total_completed_.fetch_add(1, std::memory_order_relaxed);
            } catch (...) {
                LOG_MODULE_ERROR(modules::SCHEDULER, "Task execution failed with unknown exception");
                total_completed_.fetch_add(1, std::memory_order_relaxed);
            } });
        }
        cv_.notify_one();
