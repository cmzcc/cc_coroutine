...
#define MODERN_CORO_DEBUG 0
    ...
    void Timer::add_timer(TimePoint time_point, std::function<void()> callback, Scheduler *scheduler_ptr)
    {
        ...
        LOG_MODULE_DEBUG(logger::modules::TIMER, "add_timer callback scheduled at {} ms from now",
...
#if MODERN_CORO_DEBUG
        LOG_MODULE_DEBUG(logger::modules::TIMER, "add_timer handle={} sched_ptr={} due_in_ms={}",
                         static_cast<void *>(handle.address()), static_cast<void *>(scheduler_ptr),
                         std::chrono::duration_cast<std::chrono::milliseconds>(time_point - std::chrono::steady_clock::now()).count());
#endif
        timer_cv_.notify_one();
    }
        ...
        while (!stop_flag_)
        {
            if (timer_queue_.empty())
            {
                timer_cv_.wait(lock, [this]
                               { return stop_flag_ || !timer_queue_.empty(); });
                continue;
            }
            ...
            {
#if MODERN_CORO_DEBUG
                LOG_MODULE_DEBUG(logger::modules::TIMER, "timer fire handle={} cb={} sched_ptr={}",
                                 static_cast<void *>(next_event.handle ? next_event.handle.address() : nullptr),
                                 static_cast<bool>(next_event.callback), static_cast<void *>(next_event.scheduler_ptr_));
#endif
                timer_queue_.pop();
                ...
                else
                {
                    if (next_event.handle)
                    {
                        scheduler->schedule([handle = next_event.handle]() mutable
                                            {
#if MODERN_CORO_DEBUG
                        LOG_MODULE_DEBUG(logger::modules::TIMER, "resuming handle={}", static_cast<void*>(handle.address()));
#endif
                        handle.resume(); });
                    }
                    if (next_event.callback)
