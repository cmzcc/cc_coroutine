...
namespace modern_coro
{
    ...
    public:
        struct promise_type
        {
            ...
            // 使用弱引用避免循环引用，改为引用计数控制
            std::weak_ptr<void> keep_alive_weak_{};
            bool self_owned_{false}; // 标记是否需要自持有
        ...
        void set_keep_alive(std::shared_ptr<void> keep)
        {
            if (state_ && state_->handle)
            {
                state_->handle.promise().keep_alive_weak_ = std::move(keep);
            }
        }
    ...
    public:
        ...
        handle_type handle() const { return state_ ? state_->handle : handle_type{}; }
        // Keep-alive helpers for scheduler - 改用弱引用避免循环引用
        std::shared_ptr<void> share_state_opaque() const { return state_; }
        void set_keep_alive(std::shared_ptr<void> keep)
        {
            if (state_ && state_->handle)
            {
                // 存储弱引用而不是强引用，避免循环依赖
                state_->handle.promise().keep_alive_weak_ = keep;
                state_->handle.promise().self_owned_ = true;
            }
        }
